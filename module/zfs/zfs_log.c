...
#ifdef __FreeBSD__
	err = zfs_dirent_lookup(tdzp, tmpname, &tmpzp, ZNEW);
#else
	err = zfs_dirent_lock(&tmpdl, tdzp, tmpname, &tmpzp, ZNEW, NULL, NULL);
#endif
...
zfs_log_rename_whiteout(zilog_t *zilog, dmu_tx_t *tx, uint64_t txtype,
    znode_t *sdzp, const char *sname, znode_t *tdzp, const char *dname,
    znode_t *szp, znode_t *wzp, vsecattr_t *vsecp, zfs_fuid_info_t *fuidp,
    vattr_t *vap)
{
	zfs_log_rename(zilog, tx, txtype, sdzp, sname, tdzp, dname, szp);
	txtype |= TX_CREATE;
