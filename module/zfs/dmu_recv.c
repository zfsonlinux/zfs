    ...
    struct receive_record_arg *rrd, blkptr_t *bp)
{
	...
	if ((drrw->drr_compressiontype != BP_GET_COMPRESS(bp)) ||
	    (!rwa->raw && BP_USES_CRYPT(bp))) {
		...
		if (!rwa->raw && BP_USES_CRYPT(bp)) {
			...
			abd_t *eabd = abd_alloc_linear(drrw->drr_logical_size,
			    B_FALSE);
			...
			err = zio_do_crypt_abd(B_TRUE, &dck->dck_key,
			    ...
			    mac, drrw->drr_logical_size, abd, eabd, &no_crypt);
		...
		if (BP_GET_COMPRESS(bp) != ZIO_COMPRESS_OFF) {
			/* Recompress the data */
			abd_t *cabd = abd_alloc_linear(drrw->drr_logical_size,
			    B_FALSE);
			zio_compress_data(BP_GET_COMPRESS(bp),
			    abd, abd_to_buf(cabd), drrw->drr_logical_size,
			    rwa->os->os_complevel);
	...
	io = zio_rewrite(NULL, rwa->os->os_spa, 0, bp, abd, BP_GET_PSIZE(bp),
	    NULL, NULL, ZIO_PRIORITY_SYNC_WRITE, flags, &zb);

	/* compute new bp checksum value and make sure it matches the old one */
	zio_checksum_compute(io, BP_GET_CHECKSUM(bp), abd,
	    drrw->drr_logical_size);
	if (drrw->drr_logical_size != BP_GET_LSIZE(bp) ||
