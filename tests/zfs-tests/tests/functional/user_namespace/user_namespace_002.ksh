...
#
# STRATEGY:
#       1. Delegate a dataset to an user namespace.
...
# 1. Create an user namespace with a cloned mount namespace, then delegate.
...
ns=$(readlink $proc_ns | sed -E -e 's,.*\[(.*)\],\1,g')
log_note "unshare: child=${unshared_pid} ns=${ns}"
if [ -n "$ns" -a "$ns" -ne "4026531837" ]; then
	log_unsupported "Failed to detect user namespace"
fi
