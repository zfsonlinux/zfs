...
# Copyrgith (c) 2020 Axcient. All rights reserved.
#
...
log_must eval "zfs send $TESTPOOL/$TESTFS1@snap2 > $backup"
corrupt_blocks_at_level $file 0
# test healing recv from a full send file
test_corrective_recv $TESTPOOL/$TESTFS1@snap2 $backup
...
log_must eval "zfs send $TESTPOOL/$TESTFS2@snap2 > $unc_backup"
...
test_corrective_recv "$TESTPOOL/testfs3@snap2" $backup
...
test_corrective_recv "$TESTPOOL/testfs4@snap2" $unc_backup
...
log_must eval "zfs send --raw $TESTPOOL/testfs5@snap2 > $raw_backup"
log_must eval "zfs send $TESTPOOL/testfs5@snap2 > $backup"
...
test_corrective_recv "$TESTPOOL/testfs5@snap2" $backup
corrupt_blocks_at_level "/$TESTPOOL/testfs5/$TESTFILE0" 0
...
test_corrective_recv "$TESTPOOL/testfs5@snap2" $raw_backup
...
log_mustnot eval "zfs recv -h -c $TESTPOOL/testfs5@snap2 < $backup"
log_mustnot eval "zfs recv -F -c $TESTPOOL/testfs5@snap2 < $backup"
log_mustnot eval "zfs recv -M -c $TESTPOOL/testfs5@snap2 < $backup"
log_mustnot eval "zfs recv -s -c $TESTPOOL/testfs6@snap2 < $backup"
log_mustnot eval "zfs recv -u -c $TESTPOOL/testfs6@snap2 < $backup"
log_mustnot eval "zfs recv -d -c $TESTPOOL/testfs6@snap2 < $backup"
log_mustnot eval "zfs recv -e -c $TESTPOOL/testfs6@snap2 < $backup"

# test that healing recv doesn't work on non-existing snapshots
log_mustnot eval "zfs recv -c $TESTPOOL/testfs5 < $backup"
log_mustnot eval "zfs recv -c $TESTPOOL/testfs5@missing < $backup"

# ensure healing recv doesn't work when snap GUIDS don't match
log_mustnot eval "zfs recv -c $TESTPOOL/testfs5@snap2 < $ibackup"
