...
{
	cleanup
	log_must set_tunable32 ZIL_SAXATTR_ENABLED $orig_zil_saxattr_enabled
}

log_assert "Verify saxattr logging in to ZIL works"

orig_zil_saxattr_enabled=$(get_tunable ZIL_SAXATTR_ENABLED)
...
function validate_zil_saxattr
{
	...
	log_must set_tunable32 ZIL_SAXATTR_ENABLED $saxattrzil
	...
	#
	# 6. Verify Xattr
	# If saxattrzil=0, then xattr=sa logging in zil is not enabled,
	# So, xattrs would be lost.
	# If saxattrzil=1, then xattr=sa logging in zil is enabled,
	# So, xattrs shouldn't be lost.
	#
	for i in $(seq $NFILES); do
		if [ $saxattrzil -eq 0 ]; then
...
#Validate xattr=sa logging in zil disabled.
validate_zil_saxattr 0

#Validate xattr=sa logging in zil enabled.
